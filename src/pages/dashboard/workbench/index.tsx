			...
			<div className="flex flex-col gap-3">
				...
				<div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
					{talimDarajalari.map((stat) => (
						<Card
							key={stat.label}
							className={`${stat.bg} border-0 py-0 rounded-2xl dark:bg-[#141414]`}
						>
							<CardContent className="flex items-center gap-4 px-5 py-4 dark:text-white">

								<Icon icon={stat.icon} size={32} className={stat.color} />

								<div className="flex flex-col gap-0.5">
									<span className="text-[12px] text-muted-foreground dark:text-gray-300 leading-tight">
										{stat.label}
									</span>

									<span className={`text-[22px] font-bold leading-tight ${stat.color}`}>
										{stat.value}
									</span>
								</div>

	...
	return [
		{
			...
			header: "#",
			cell: ({ row }) => (
				<span className="text-muted-foreground dark:text-slate-400">{row.getValue("id")}</span>
			),
		},
		{
			accessorKey: "image",
			header: "Rasm",
			cell: ({ row }) => (
				<div className="w-9 h-9 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-700 dark:text-blue-400 font-bold text-[13px]">
		...
		},
		{
			...
			header: "Fakultet",
			cell: ({ row }) => (
				<span className="font-medium dark:text-slate-200">{row.getValue("name")}</span>
			),
		},
		{
			id: "actions",
			header: () => <div className="text-center dark:text-slate-200">Amallar</div>,
			cell: ({ row }) => (
				<div className="flex items-center justify-center gap-2">
					<button
						type="button"
						onClick={() => onEdit(row.original)}
						className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/40 text-[12px] font-semibold px-2.5 py-1 rounded-md transition-colors cursor-pointer"
					>
					...
					<ConfirmPopover onConfirm={() => onDelete(row.original)}>
						<button		
							type="button"
							className="inline-flex items-center gap-1.5 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 text-[12px] font-semibold px-2.5 py-1 rounded-md transition-colors cursor-pointer"
						>
...
>
	...
	>
		...
		<div className="flex flex-col gap-2">
			<Label htmlFor="faculty-name" className="dark:text-white">
				Fakultet nomi
			</Label>
